...
class Filter(nn.Module):
    ...
    def Up_op(self, x, i_lvl):
        UpSig = torch.zeros(
            x.size(dim=0), 1, self.layerSize[i_lvl], dtype=torch.float64, device=device)
    ...
    def Biases_gen(self, Archi, initHT, trainHT):
        ...
        if Archi == "WPT":
            for i in range(self.level):
                ...
                for j in range(int(2**i)):
                    biases.append(nn.Parameter(
                        data=torch.tensor(
                            np.array([initHT])[np.newaxis, np.newaxis, :].copy(), dtype=torch.float64, device=device).clone(),
            ...
            for i in range(self.level):
                ...
                biases.append(nn.Parameter(
                    data=torch.tensor(
                        np.array([initHT])[np.newaxis, :].copy(), dtype=torch.float64, device=device).clone(),
                    requires_grad=trainHT))
                biases.append(nn.Parameter(
                    data=torch.tensor(
                        np.array([initHT])[np.newaxis, :].copy(), dtype=torch.float64, device=device).clone(),
